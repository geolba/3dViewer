define('lib/proj4js/proj4js-amd', [],
function () {
    "use strict";
    /**
    * @preserve
     ...
     */
     ...
     parseWKT: function(wkt) {
        ...
        */
        ...
        for (var k=0; k<wktArray.length; ++k) {
          this.parseWKT(wktArray[k]);
        }
     },
    ...
    */
    ...
    //Proj4js.defs["EPSG:102100"] = "+proj=merc +a=6378137 +b=6378137 +lat_ts=0.0 +lon_0=0.0 +x_0=0.0 +y_0=0 +k=1.0 +units=m +nadgrids=@null +wktext  +no_defs";
    ...
    //Proj4js.defs["EPSG:102100"] = Proj4js.defs['EPSG:3785'];
    ...
    Proj4js.defs["EPSG:31297"] = "+proj=lcc +lat_1=49 +lat_2=46 +lat_0=47.5 +lon_0=13.33333333333333 +x_0=400000 +y_0=400000 +ellps=bessel +towgs84=426.9,142.6,460.1,4.91,4.49,-12.42,17.1 +units=m +no_defs";
    ...
    */
    Proj4js.datum = Proj4js.Class({
      ...
      compare_datums : function( dest ) {
        ...
        //    return this.strcmp( this.pj_param(this.params,"snadgrids").s, this.pj_param(dest.params,"snadgrids").s ) === 0;
        //}
        else {
     ...
     * "Institut fuer Erdmessung", University of Hannover, July 1988.
    ...
    *******************************************************************************/
    ...
    /* Albers Conical Equal Area forward equations--mapping lat,long to x,y
      ...
      //  if (this.e3 >= 1e-10) {
      //    con = 1 - 0.5 * (1.0 -this.es) * Math.log((1.0 - this.e3) / (1.0 + this.e3))/this.e3;
    ...
    -------------------------------------------*/
      phi1z: function (eccent,qs) {
        ...
        for (var i = 1; i <= 25; i++) {
            ...
            con = eccent * sinphi; 
            com = 1.0 - con * con;
            dphi = 0.5 * com * com / cosphi * (qs / (1.0 - eccnts) - sinphi / com + 0.5 / eccent * Math.log((1.0 - con) / (1.0 + con)));
            phi = phi + dphi;
    ...
    Proj4js.Proj.sterea = {
      ...
      forward : function(p) {
        ...
        var sinc = Math.sin(p.y);
        var cosc = Math.cos(p.y);
        var cosl = Math.cos(p.x);
        var k = this.k0 * this.R2 / (1.0 + this.sinc0 * sinc + this.cosc0 * cosc * cosl);
    ...
    //	var sinphi, sin2ph,  ml, mlp, con1, con2, con3, dphi, i;
    ...
    Proj4js.Proj.nzmg = {
      ...
      */
      forward : function(p) {
        ...
        var x = (z_im * this.a) + this.x0;
        var y = (z_re * this.a) + this.y0;
    ...
    *******************************************************************************/
    ...
    Proj4js.Proj.omerc = {
      ...
      forward: function(p) {
        //var theta;		/* angle					*/
        var sin_phi;/* sin and cos value				*/
        var t;	/* temporary values				*/
        var con;	/* cone constant, small m			*/
        var q,us,vl;
        var ul,vs;
        var s;
        var dlon;
